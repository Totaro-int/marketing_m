#!/usr/bin/env node
// guide-pdf.mjs — docs/INSTALL-CLIENT.md → docs/INSTALL-CLIENT.pdf (이미지 포함, 헤드리스 크롬 print).
...
const PORT = 5185;
const MIME = { '.html': 'text/html', '.md': 'text/plain; charset=utf-8', '.png': 'image/png', '.css': 'text/css' };
...
code{background:#f3f1 ea;background:#f5f3ee;padding:2px 6px;border-radius:5px;font-size:12px;color:#9c5b00}
...
img{max-width:100%;border:1px solid #e6e3dc;border-radius:10px;margin:10px 0;box-shadow:0 2px 12px rgba(0,0,0,.08)}
...
<script>fetch('INSTALL-CLIENT.md',{cache:'no-store'}).then(r=>r.text()).then(t=>{document.getElementById('md').innerHTML=marked.parse(t);window.__ready=true;});</script>
</body></html>`;
...
} finally { await browser.close(); srv.close(); fs.rmSync(tmp, { force: true }); }
