...
async function main() {
  ...
  ui.dim(`  feedback ${feedback.length}건 · 활성 지침 ${sources.length}개`);

  if (DRY) { ui.ok(`[DRY-RUN] 받을 데이터: feedback ${feedback.length} · guidelines ${sources.length} (저장 안 함)`); return; }
  ...
  ui.ok(`완료 — feedback → learnings/_inbox-feedback.json · 지침 → brand/_pulled-guidelines.txt. 다음: node scripts/distill.mjs`);
}
