...
// 색인 로직은 내장 공유 모듈(engine/seo.mjs)로 일원화 — seo-index CLI 와 동일 코드.
import { updateSitemap, writeLlmsTxt, ensureIndexNowKey, pingIndexNow, INDEXNOW_KEY, BASE_URL } from '../engine/seo.mjs';
...
async function main() {
  ...
  if (art.guard.blocked) {
    ...
    const sm = updateSitemap(siteRoot, [`${BASE_URL}/insights`, `${BASE_URL}/insights/${date}`]);
    writeLlmsTxt(siteRoot, arr);
    ensureIndexNowKey(siteRoot);
